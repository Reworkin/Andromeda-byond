...
/turf/closed/wall/r_wall/get_ru_names()
	return list(
		...
		ACCUSATIVE = "усиленную стену",
		INSTRUMENTAL = "усиленной стеной",
...
/turf/closed/wall/r_wall/deconstruction_hints(mob/user)
	switch(d_state)
		...
		if(SUPPORT_LINES)
			return span_notice("Внешняя <i>решётка</i> разрезана, а опорные линии <b>привинчены</b> к внешней обшивке.")
		if(COVER)
			return span_notice("Опорные линии <i>отвинчены</i>, а металлическая обшивка <b>приварена</b> намертво.")
		if(CUT_COVER)
			return span_notice("Металлическая обшивка <i>разрезана</i> и <b>слабо прикреплена</b> к каркасу.")
		if(ANCHOR_BOLTS)
			return span_notice("Внешняя обшивка <i>отодвинута</i>, а болты крепления опорных стержней <b>закручены</b>.")
		if(SUPPORT_RODS)
			return span_notice("Болты крепления опорных стержней <i>ослаблены</i>, но всё ещё <b>приварены</b> к каркасу.")
		if(SHEATH)
			return span_notice("Опорные стержни <i>перерезаны</i>, а внешняя оболочка <b>слабо прикреплена</b> к каркасу.")
		...
		if(SUPPORT_LINES)
			if(W.tool_behaviour == TOOL_SCREWDRIVER)
				...
				if(W.use_tool(src, user, 40, volume=100))
					...
					d_state = COVER
					update_appearance()
					to_chat(user, span_notice("Вы откручиваете опорные линии."))
				return TRUE
		...
		if(COVER)
			if(W.tool_behaviour == TOOL_WELDER)
				if(!W.tool_start_check(user, amount=2, heat_required = HIGH_TEMPERATURE_REQUIRED))
					return
				to_chat(user, span_notice("Вы начинаете прорезать металлическую обшивку..."))
				if(W.use_tool(src, user, 60, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != COVER)
						return TRUE
					d_state = CUT_COVER
					update_appearance()
					to_chat(user, span_notice("Вы с силой нажимаете на обшивку, смещая её."))
				return TRUE

			if(W.tool_behaviour == TOOL_SCREWDRIVER)
				to_chat(user, span_notice("Вы начинаете закручивать опорные линии..."))
				if(W.use_tool(src, user, 40, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != COVER)
						return TRUE
					d_state = SUPPORT_LINES
					update_appearance()
					to_chat(user, span_notice("Опорные линии закреплены."))
				return TRUE

		if(CUT_COVER)
			if(W.tool_behaviour == TOOL_CROWBAR)
				to_chat(user, span_notice("Вы пытаетесь отодвинуть обшивку..."))
				if(W.use_tool(src, user, 100, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != CUT_COVER)
						return TRUE
					d_state = ANCHOR_BOLTS
					update_appearance()
					to_chat(user, span_notice("Вы отодвигаете обшивку."))
				return TRUE

			if(W.tool_behaviour == TOOL_WELDER)
				if(!W.tool_start_check(user, amount=2, heat_required = HIGH_TEMPERATURE_REQUIRED))
					return
				to_chat(user, span_notice("Вы начинаете приваривать металлическую обшивку обратно к каркасу..."))
				if(W.use_tool(src, user, 60, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != CUT_COVER)
						return TRUE
					d_state = COVER
					update_appearance()
					to_chat(user, span_notice("Металлическая обшивка приварена к каркасу."))
				return TRUE

		if(ANCHOR_BOLTS)
			if(W.tool_behaviour == TOOL_WRENCH)
				to_chat(user, span_notice("Вы начинаете ослаблять крепёжные болты, фиксирующие опорные стержни..."))
				if(W.use_tool(src, user, 40, volume=100))
			...
			if(W.tool_behaviour == TOOL_CROWBAR)
				to_chat(user, span_notice("Вы пытаетесь вернуть обшивку на место..."))
				if(W.use_tool(src, user, 20, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != ANCHOR_BOLTS)
						return TRUE
					d_state = CUT_COVER
					update_appearance()
					to_chat(user, span_notice("Металлическая обшивка возвращена на место."))
				return TRUE
		...
		if(SUPPORT_RODS)
			...
			if(W.tool_behaviour == TOOL_WRENCH)
				...
				if(W.use_tool(src, user, 40))
					...
					d_state = ANCHOR_BOLTS
					update_appearance()
					to_chat(user, span_notice("Вы затягиваете болты, крепящие опорные стержни."))
				return TRUE

		if(SHEATH)
			if(W.tool_behaviour == TOOL_CROWBAR)
				to_chat(user, span_notice("Вы пытаетесь отодвинуть внешнюю оболочку..."))
				if(W.use_tool(src, user, 100, volume=100))
					if(!istype(src, /turf/closed/wall/r_wall) || d_state != SHEATH)
						return TRUE
					to_chat(user, span_notice("Вы отодвигаете внешнюю оболочку."))
					dismantle_wall()
				return TRUE
...
/turf/closed/wall/r_wall/plastitanium/syndicate/get_ru_names()
	return list(
		...
		DATIVE = "корпусу",
		ACCUSATIVE = "корпус",
		INSTRUMENTAL = "корпусом",
		PREPOSITIONAL = "корпусе"
	)
